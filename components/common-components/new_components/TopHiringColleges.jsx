...
const TopHiringColleges = () => {
  ...
  useEffect(() => {
    const handleLoginSuccess = () => collgeList();
  ...
  const collgeList = async () => {
    try {
      ...
      // Get user college_ids if logged in
      let userCollegeIds= [];
      ...
      if (profileStr) {
        ...
        try {
          const response = await Models.profile.details(profile.id);
          console.log('collgeList --->', response);
          userCollegeIds = response?.college_ids || [];
        } catch (e) {
      ...
      console.log('✌️allColleges --->', allColleges);


      // Filter by user college_ids if available
      const finalList = userCollegeIds.length > 0
        ? allColleges.filter((c) => userCollegeIds.includes(c.id))
        : allColleges;
