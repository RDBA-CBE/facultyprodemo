...
const FindYourJob = () => {
  ...
  useEffect(() => {
    const handleLoginSuccess = () =>
      fetchUserCollegeIds().then((ids) => jobList(1, ids));
  ...
  const fetchUserCollegeIds = async (): Promise<number[]> => {
    const profileStr = localStorage.getItem("user");
    if (!profileStr) return [];
    try {
      const profile = JSON.parse(profileStr);
      const response: any = await Models.profile.details(profile.id);
      console.log('✌️response --->', response);
      const ids: number[] = response?.college_ids || [];
      setState({ userCollegeIds: ids });
      return ids;
    } catch (e) {
  ...
  const jobList: any = async (page = 1, collegeIds?: number[]) => {
    try {
      setState({ loading: true });
      const body = bodyData(collegeIds);
  ...
  const bodyData = (collegeIds?: number[]) => {
    const body: any = {};
    if (debouncedSearch) body.search = debouncedSearch;
    if (state?.location) body.location = state.location;
    if (state?.JobCat) body.category = state.JobCat;
    const ids = collegeIds ?? state?.userCollegeIds;
    if (ids?.length > 0) body.colleges = ids;
    return body;
  };
                            ...
                            <div onClick={(e) => e.stopPropagation()}>
                              <RWebShare
                                data={{
                                  title: "Faculty Plus",
                                  text: "Check this out!",
                                  url: `${FRONTEND_URL}/jobs/${job.slug}`,
                                }}
                              >
