...
type SlugResolvedFilters = Partial<{
  locations: number[];
  colleges: number[];
  department: number[];
  ...
  jobRole: number[];
  categories: number[];
  academic_responsibilities: number[];
}>;

const cascadeFilterMappings: Array<{
  responseKey: string;
  filterKey: keyof SlugResolvedFilters;
}> = [
  { responseKey: "locations", filterKey: "locations" },
  { responseKey: "colleges", filterKey: "colleges" },
  { responseKey: "departments", filterKey: "department" },
  { responseKey: "experiences", filterKey: "experienceLevels" },
  { responseKey: "salary_ranges", filterKey: "salaryRange" },
  { responseKey: "job_roles", filterKey: "jobRole" },
  { responseKey: "job_categories", filterKey: "categories" },
  {
    responseKey: "additional_academic_responsibilities",
    filterKey: "academic_responsibilities",
  },
];
...
/** Finds every cascade facet whose display value matches a segment in the jobs URL. */
function resolveFiltersFromUrl(
  cascadeData: Record<string, any> | undefined,
  pathname: string,
): SlugResolvedFilters {
  if (!cascadeData) return {};

  const urlTerms = pathname
    .split("/")
    .filter(Boolean)
    .slice(1) // omit the `jobs` segment
    .map(normalizeUrlSegment)
    .filter(Boolean);
  if (!urlTerms.length) return {};

  const resolved: SlugResolvedFilters = {};
  for (const { responseKey, filterKey } of cascadeFilterMappings) {
    const matches: number[] = ((cascadeData[responseKey] ?? []) as Record<
      string,
      unknown
    >[])
      .filter((item: Record<string, unknown>) => {
        const values = Object.entries(item)
          ...
          .map(([, value]) => normalizeUrlSegment(String(value)));
        return values.some((value) => urlTerms.includes(value));
      })
    ...
    if (matches.length) resolved[filterKey] = [...new Set(matches)];
  }
  return resolved;
}
...
${userName}`;
  ...
  // SEO/listing URLs do not include filter IDs. Resolve their slugs from the
  // cascade response once, before applying the matching IDs as filters.
  const slugFilterResolutionAttemptedRef = useRef(false);
  ...
  const filterList = async () => {
    try {
      const shouldResolveUrlSlug =
        !slugFilterResolutionAttemptedRef.current &&
        pathname.split("/").filter(Boolean).length > 1;
      if (shouldResolveUrlSlug) slugFilterResolutionAttemptedRef.current = true;

      // The first cascade call for a slug URL deliberately has no filters.
      // Its facets are the source of truth for resolving URL text to IDs.
      const body = shouldResolveUrlSlug ? {} : bodyData();
      const res: any = await Models.job.filterList(body);

      if (shouldResolveUrlSlug) {
        const resolvedFilters = resolveFiltersFromUrl(res?.data, pathname);
        if (Object.keys(resolvedFilters).length > 0) {
          setFilters((previousFilters) => {
            const nextFilters = { ...previousFilters };
            for (const [key, ids] of Object.entries(resolvedFilters)) {
              const filterKey = key as keyof SlugResolvedFilters;
              // Explicit query-param/user-selected filters always win.
              if ((previousFilters[filterKey] as number[])?.length === 0) {
                (nextFilters[filterKey] as number[]) = ids as number[];
              }
            }
            return nextFilters;
          });
      ...
      const allowedCollegeIds = Array.isArray(collegeIdRef.current)
        ? new Set(collegeIdRef.current)
        : null;
      const collegeList = res?.data?.colleges
        ?.filter(
          (item) =>
            !allowedCollegeIds ||
            allowedCollegeIds.size === 0 ||
            allowedCollegeIds.has(item.id),
        )
        .map((item) => ({
          value: item.id,
...
const filteredData = res?.results?.filter((item) =>
  ...
  const bodyData = () => {
    ...
    } else if (Array.isArray(collegeIdRef.current) && collegeIdRef.current.length > 0) {
      // Pass user's college_ids as default filter when no manual college filter applied
      body.colleges = collegeIdRef.current;
    }
                          ...
                          >
                            ...
                            {state.jobList?.map((job: any) => (
                              <div
                                ...
                                // }}
                                onClick={() => {
                                  // Mobile job cards navigate straight to the dedicated detail
                                  // page. Do not populate `selectedJob`, which would open the
                                  // legacy mobile bottom sheet during route navigation.
                                  if (isMobileScreen) {
                                    router.push(
